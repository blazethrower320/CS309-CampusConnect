...
POST request: 
    /people - Create a new inventory. The request has to be of type application/JSON input format 
    {
        "firstName" : "tom",
        "lastName"  : "hanks",
        "address"   : "hollywood",
...
2. READ info on a inventory
GET request:
    /people/{firstName} - Get all inventory whose name is the same as the parameter {firstName} (Example: /people/tom )

GET request:
    /people/contains?name={....} - Get all names who contains {name} in either firstName or lastName (Example: /people/contains?name=Bob)

3. UPDATE info on a inventory
PUT request : 
    /people/{firstname} - Update info on a inventory whose firstname is sent on the URL. The modified info is sent in the body of the message
    {
        "firstName" : "tom",
        "lastName"  : "hanks",
        "address"   : "hollywood",
...
PUT request :
    /people?firstName={....} - Update info on a inventory whose firstName matches the request parameter. The modified info is sent in the body of the message
    {
        "firstName" : "tom",
        "lastName"  : "hanks",
        "address"   : "hollywood",
